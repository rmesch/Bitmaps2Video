...
/// <summary> Use to list the long names of the codecs supported by the file format defined by Ext.
...
function GetOutputFormat(const Ext: string): PAVOutputFormat;
begin
  Result := av_guess_format(nil, PAnsiChar(AnsiString('Example' + Ext)), nil);
end;
...
begin
  ...
  Result := av_guess_codec(fmt, nil, PAnsiChar(AnsiString('Example' + Ext)),
...
begin
  ...
  for CodecId in EncoderDictionary.Keys do
    ...
    begin
      ...
      if Codec <> nil then
      begin
        Strings.AddObject(String(Codec.long_name), TObject(CodecId));
...
//var ret: integer; qual: Ansistring;
begin
  Result:=inherited QualityToBitrate(Quality,Width,Height,rate);
   //The quality could be set by av_dict_set, but the bitrate is more predictable.
